							...
							con.query(query, function (err, result) {
								if (!err) {
									...
									if (element.details != undefined) {
										...
										var MySql = require('sync-mysql'); //  usare un solo accesso al db
											...
											if ((typeof details === "object") && (details !== null) && (! Array.isArray(details))) {
												...
												if (isComplex(params)) {
													...
													allPossibleCases(params,0).forEach(function(element){
														...
														var replacedQuery = details.query;
														
														for (var i = 0; i!= element.length; i++) {
															replacedQuery = replaceAll(replacedQuery, "@"+i, element[i]);
														}
														
														var resultDB = connection.query(replacedQuery, []);
													...
													for (var i = 0; i!= params.length; i++) {
														replacedQuery = replaceAll(replacedQuery, "@"+i, params[i]);
													}
										...
										msg.payload = { url: element.url, source: element.source, description: element.description, chart: { data: result, chartOptions: element.chartOptions }, details: solveJSON(element.details)};
									
									}
									else {
										msg.payload = { url: element.url, source: element.source, description: element.description, chart: { data: result, chartOptions: element.chartOptions }};
									}
									node.send(msg);
									count += 1;
